...
import { AiOutlineDelete, AiFillPlusCircle, AiOutlineDown,AiOutlineUp } from "react-icons/ai";
...
export default function Community() {
	...
	return (
		<Layout title={'Community'} className={'Community'}>
			<button className="openToggleButton" onClick={()=>setOpen(!Open)}>+</button>
					<nav className='pagination'>
						{Array(pageNum.current).length > 0 && Array(pageNum.current)
							.fill()
							.map((_, idx) => {
								return (
							...
							return (
								<article key={el + idx} className={`commentBox ${el.src}`}>
									<div className='txt'>
										<div className='top'>
											<div className='img'>
												<img src={`${path.current}/img/${el.src}.jpg`} alt=""/>
											</div>
											<p>{el.nickname}</p>
										</div>
										<h2>{el.title}</h2>
										...
										<span>{strDate}</span>
										<span onClick={(e)=>handleReplyView(e, idx)}>replay view{el.replyView ?<AiOutlineUp /> : <AiOutlineDown /> }</span>
									...
									{el.onReply &&
										<label htmlFor="replyInput" className='replyInput'>
											<input type="text" id="replyInput" placeholder='reply' ref={refReply}/>
											<button type="button" onClick={(e)=>handleInputChange(e, idx)}>add reply</button>
										</label>
										}
									<nav>
										<button className={el.onReply ? 'onReply on': 'replyOn'} onClick={() => addReply(idx)}><AiFillPlusCircle /></button>
										<button onClick={() => enableUpdate(idx)}><LiaEdit /></button>
										<button onClick={() => deletePost(idx)}><AiOutlineDelete /></button>
									</nav>
								</article>
							);
						} else {
